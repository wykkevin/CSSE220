...
public class Fan {
	...
	public void drawOn(Graphics2D graphics2) {
		...
		for (int i = 0; i < this.numberOfBlades; i++) {
			graphics2.rotate(Math.toRadians((360 / this.numberOfBlades) * i));
			Rectangle2D.Double oneBlade = new Rectangle2D.Double(-this.bladeLength * BLADE_ASPECT_RATIO / 2,
					-this.bladeLength, this.bladeLength * BLADE_ASPECT_RATIO, this.bladeLength);
			
			graphics2.setColor(Color.red);
			graphics2.fill(oneBlade);
			graphics2.setColor(Color.black);
			graphics2.draw(oneBlade);
			graphics2.rotate(-Math.toRadians((360 / this.numberOfBlades) * i));
		}
		graphics2.translate(-centerX, -centerY);
