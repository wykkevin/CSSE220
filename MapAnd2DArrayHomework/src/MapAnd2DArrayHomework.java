...
/**
 * This class includes problems about 2d arrays and hashmaps.
 *
 * @author Yuankai Wang. Created Dec 3, 2016.
 */
...
public class MapAnd2DArrayHomework {
	...
	public static char mostCommonCharacter(String input) {
		...
		for (int i = 0; i < input.length(); i++) {
			...
			if (!map.containsKey(inputchar)) {
				...
				map.put(inputchar, map.get(inputchar) + 1);
			}
			if (map.get(inputchar) > max) {
				max = map.get(inputchar);
				output = inputchar;
			}
		}
		return output;
	...
	public static HashMap<String, ArrayList<Integer>> reverseHashmap(HashMap<Integer, String> initialMap) {
		...
		for (int values : initialMap.keySet()) {
			ArrayList<Integer> tempList = new ArrayList<Integer>();
			if (!output.containsKey(initialMap.get(values))) {
				tempList.add(values);
				output.put(initialMap.get(values), tempList);
			} else {
				output.get(initialMap.get(values)).add(values);
			}
	...
	public static String getTemperatureDropCity(int[] temps, String[] tempCity) {
		for (int i = 0; i < temps.length; i++) {
			for (int j = i; j < temps.length; j++) {
				if (tempCity[j].equals(tempCity[i])) {
					if (temps[j] < temps[i]) {
						return tempCity[j];
					}
				}
	...
	public static int getNumberOfCoursesToTake(HashMap<String, String> courseMap, String course) {
		int count = 0;
		while (courseMap.get(course).equals("") == false) {
			course = courseMap.get(course);
			count++;
			if (count > courseMap.keySet().size()) {
				return -1;
			}
		}
		return count;
	...
	/**
	 * 
	 * I divide the places that are continue in several conditions. First is for
	 * the main body part. Second is the startings and endings.
	 *
	 * @param data
	 * @return
	 */
		...
		for (int i = 0; i < data.length; i++) {
			...
			for (int j = 0; j < data[0].length - 1; j++) {
				if (data[i][j] == data[i][j + 1]) {
					count++;
					if (i < data.length - 1 && data[i + 1][0] == data[i][data[i].length - 1]) {
						count++;
					}
					if (i > 0 && j == 0 && data[i][j] == data[i - 1][data[i].length - 1]) {
						count++;
					}
				}
			}
	...
	public static int ballRestElevation(int[][] map) {
		int i = 1;
		int j = 1;
		while (map[i][j] > map[i + 1][j] || map[i][j] > map[i - 1][j] || map[i][j] > map[i][j + 1]
				|| map[i][j] > map[i][j - 1]) {
			int a = map[i][j] - map[i + 1][j];
			int b = map[i][j] - map[i - 1][j];
