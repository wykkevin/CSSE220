...
/**
 * 
 * A class designed to startup some threads, and safely shutdown when they are
 * complete.
 *
...
public class StartupShutdown {
	 ...
	 * 
	 * You should modify it so that it starts the job in its own thread so that
	 * many jobs can run at the same time.
	 */
	public void handleStartup() {
		class StartJob implements Runnable {
		...
		Thread t = new Thread(new StartJob());
		this.threadList.add(t);
		t.start();
	}

	/**
	 * This function should wait for all currently running threads to finish,
	 * then exit.
	 * 
	 */
	 ...
	 * 
	 * You can modify this function if you wish but you probably shouldn't have
	 * to.
	 */
	public void doAJob() {
